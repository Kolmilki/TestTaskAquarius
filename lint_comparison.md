...
```
Функции проверки только тех строк, методов или функций которые были изменены у Linter-ов нет. Для исправления этой ситуации приходится писать 
shell скрипты и многократно обращаться к методами git. В git изменения кода отлично видны и можно даже выяснить менялись ли какие-то определенные 
атрибуты или нет: ```git diff -g<regex>```
---
