...
from filling_json import choosing_mode, forming_json_file
...
def read_configuration_file(configuration_file, configuration_setting):
    ...
    print(
        ...
        if f"#{configuration_setting}\n" in content
        else "Конфигурация не найдена"
    )
...
def remove_directories():
    ...
    files = [
        ...
        "b.txt",
        "c.txt",
        "fileDirectoryDE/d.txt",
        "fileDirectoryDE/e.txt",
        "fileDirectoryFGH/f.txt",
        "fileDirectoryFGH/g.txt",
        "fileDirectoryFGH/h.txt",
    ]
    for path in files:
...
if __name__ == "__main__":
    ...
    if k > x:
        k, x = x, k
    config_number_for_json, mode_for_json, path_for_json = read_configuration_file(
        configuration_file, configuration_setting
    )
    forming_json_file(
        configuration_file, config_number_for_json, mode_for_json, path_for_json
    )
